...
public class WD_Copy extends HTTPCommand
{
    public WD_Copy()
    {
        super("copy");
    ...
    public HTTPMessage Compute(HTTPMessage input, HTTPEnvironment environment)
    {
        try
        {
            ...
            IResource fsrc = environment.createFromPath(environment.getRoot() + input.getPath().trim());

            if(!fsrc.exists())
                return new HTTPMessage(404, "Not found");

            IResource fdest = environment.createFromPath(environment.getRoot() + shortDest.trim());
